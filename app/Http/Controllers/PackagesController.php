...
class PackagesController extends Controller {
	...
	function getUpdate(Request $request, $id = null)
	{
		...
		$this->date['roomTypes'] = Roomtypes::all();
	...
	public function getShow( Request $request, $id = null)
	{
		...
		if(CNF_DEF_CURRENCY){
			$tickets_cost = \DB::select('select sum(tickets.price / default_rate) sum from tickets left join def_currency on tickets.currencyID = def_currency.currencyID where TicketID in '.$ticketIDstr){0}->sum;
			$transfer_cost = \DB::select('select sum(def_vehicle.cost / default_rate) sum from tour_features left join def_vehicle on tour_features.vehicleID = def_vehicle.vehicleID left join def_currency on def_vehicle.currencyID = def_currency.currencyID where packageID = '.$id){0}->sum;
			...
			$turnover = ($tickets_cost * $total+$transfer_cost+$hotel_cost+$expense_cost+$visa_cost+$extra_cost)*$rate;
			...
			$transfer_cost = \DB::select('select sum(def_vehicle.cost / realtime_rate) sum from tour_features left join def_vehicle on tour_features.vehicleID = def_vehicle.vehicleID left join def_currency on def_vehicle.currencyID = def_currency.currencyID where packageID = '.$id){0}->sum;
  ...
  static public function travelersDetail( $traveler = '', $packageID)
	{
		...
		if($traveler !='')
		{
			...
			$travelersDetail .= "<div class='col-md-3'><a href='".url('travellers/show')."/".$traveler."'>".$nameandsurname."</a></div><div class='col-md-3'>".number_format((float)$payment, 2)." ".$currency->symbol."</div><div class='col-md-3'>".number_format((float)$InvTotal, 2)." ".$currency->symbol."</div><div class='col-md-3'>".InvoiceStatus::package_payments($payment, $InvTotal)."</div>";
		}
