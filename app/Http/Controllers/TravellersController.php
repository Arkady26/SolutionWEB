...
class TravellersController extends Controller {
	...
	public function getShow( Request $request, $id = null)
	{
		...
		$quotation 		=\DB::table('quotation')->where('travellerID',$id)->orderBy('DateIssued','ASC')->get();
		$quota = array();
		$fourth_quota = 0;
		foreach($quotation as $quo)
		{
			$quota[] = array(
				...
				'status'	        =>$quo->status,
				'travellerID'	    =>$quo->travellerID,
				'bookingID'	        =>$quo->bookingID,
				'InvTotal'	        =>$quo->InvTotal,
				'currency'	        =>$quo->currency,
				'DateIssued'        =>$quo->DateIssued,
				'untildate'       	=>$quo->untildate
			);
			++$fourth_quota;
		}
