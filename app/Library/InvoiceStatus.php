...
class InvoiceStatus
{
    ...
    static function package_payments($payment , $InvTotal)
    {
        // var_dump("test");exit;
        if ($payment ==0)
        {
            return '<spin class="btn btn-block btn-xs btn-danger" disabled>'.Lang::get('core.nopaymentmade').'</spin>';

        }elseif($payment == $InvTotal)

        {
            return '<spin class="btn btn-block btn-xs btn-success" disabled>'.Lang::get('core.fullypaid').'</spin>';
        } elseif($payment < $InvTotal)

        {
            return '<spin class="btn btn-block btn-xs btn-warning " disabled>'.Lang::get('core.partiallypaid').'</spin>';
        }
