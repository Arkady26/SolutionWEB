...
<div class="box box-primary">
	...
	<div class="box-body">
@endif
			{!! Form::open(array('url'=>'bookteam/save/'.\App\Library\SiteHelpers::encryptID($row['bookingID']), 'class'=>'form-horizontal','files' => true , 'parsley-validate'=>'','novalidate'=>' ','id'=> 'bookteamFormAjax')) !!}
			<div class="col-md-12">
				...
				.kbw-signature { width: 100%; height: 400px; }
						...
						<fieldset>
							<legend> {{Lang::get('core.assignteam')}}</legend>
							{!! Form::hidden('roomID', $row['roomID']) !!}
							{!! Form::hidden('bookingID', app('request')->input('bookingID')) !!}
								<div class="form-group  " >
								<label for="Teams" class=" control-label col-md-4 text-left"> {{ Lang::get('core.teams') }} <span class="asterix"> * </span></label>
								<div class="col-md-3">
									<select name='team' rows='5' id='team' class='select2 ' required  ></select>
								 </div>
								 <div class="col-md-2">
...
$(document).ready(function() {
	...
	$('.removeMultiFiles').on('click',function(){
		var removeUrl = '{{ url("bookteam/removefiles?file=")}}'+$(this).attr('url');
		$(this).parent().remove();
		$.get(removeUrl,function(response){});
		$(this).parent('div').empty();
		return false;
	});
	$("#team").jCombo("{!! url('tours/comboselect?filter=teams:id:name&limit=WHERE:status:=:1') !!}",
	{  selected_value : '{{$booking['team']}}' });
	var form = $('#bookteamFormAjax');
	form.parsley();
