...
@if ( $row->extraservices == 1 )
            ...
            @endif
              					@endif
                        <li class="list-group-item">
                      					 <a href="{{ url('bookteam/update?bookingID='.$id)}}"  onclick="MmbModal(this.href,'{{Lang::get('core.assignteam')}}'); return false;"  class="btn btn-primary btn-social btn-block "><i class="fa fa-edit"></i> {{Lang::get('core.assignteam')}}</a>
                       </li>
                        <li class="list-group-item">
            ...
            <div class="box-body">
             ...
             @foreach($trs as $ts)
                <tr>
                  ...
                  @else
                  <td><a href="{{ url('tours/show/'.$ts['tourID'])}}" target="_blank"><strong>{{ \App\Library\SiteHelpers::formatLookUp($ts['packageID'],'packageID','1:packages:packageID:package_name') }}</strong></a></td>
                  <td><a href="{{ url('tourdates/show/'.$ts['tourdateID'])}}" target="_blank"><strong> {{ \App\Library\SiteHelpers::formatLookUp($ts['packageID'],'packageID','1:packages:packageID:tour_code') }}</strong></a></td>
                  <td>{{date("".CNF_DATE."",strtotime(\App\Library\SiteHelpers::formatLookUp($ts['packageID'],'packageID','1:packages:packageID:start'))) }}</td>
                  <td>{{ date("".CNF_DATE."",strtotime(\App\Library\SiteHelpers::formatLookUp($ts['packageID'],'packageID','1:packages:packageID:end'))) }}</td>
                  @endif
             ...
             @foreach($flight as $fl)
                ...
                <tr>
                  <td>
                      ...
                      @if($fl['travellersID']=="" || $fl['travellersID'] == null)
                          <p>nobody</p>
                      @else
                        {!! \App\Library\SiteHelpers::showTravellers($fl['travellersID']) !!}
                      @endif
                    </td>
                  ...
                  <td>
                    @foreach($airlines as $a)
                        ...
                        <?php
                        $airlineIds = json_decode($fl['airlineID']);
                        // var_dump($fl); exit;
                        if(!is_array($airlineIds)){
                          $airlineIds = [$fl['airlineID']];
                        }
