			...
			<div class="col-md-12">
				...
				{!! Form::hidden('extraserviceID', $row['extraserviceID']) !!}
						...
						required     class='form-control ' />
									  ...
									  <div class="form-group  " >
										<label for="buy" class=" control-label col-md-3 text-left"> {{ Lang::get('core.buy') }} <span class="asterix"> * </span></label>
										<div class="col-md-3">
										  <input  type='text' name='buy' id='buy' value='{{ $row['buy'] }}' required     class='form-control ' />
										 </div>
										 <label for="Currency" class=" control-label col-md-2 text-left"> {{ Lang::get('core.currency') }} <span class="asterix"> * </span></label>
 										<div class="col-md-3">
 										  <select name='currencyID_buy' rows='5' id='currencyID_buy' class='select2 ' required  ></select>
 										 </div>
									  </div>
										<div class="form-group  " >
										<label for="Price" class=" control-label col-md-3 text-left"> {{ Lang::get('core.price') }} <span class="asterix"> * </span></label>
										 <div class="col-md-3">
...
$(document).ready(function() {
		...
		$("#currencyID_buy").jCombo("{!! url('extraservices/comboselect?filter=def_currency:currencyID:currency_sym|symbol&limit=WHERE:status:=:1') !!}",
		{  selected_value : '{{ $row["currencyID_buy"] }}' });
...
<script>
    $("input[name='price'], input[name='buy']").TouchSpin();
</script>
