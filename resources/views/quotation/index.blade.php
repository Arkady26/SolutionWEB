...
<div class="box box-primary">
	...
	<div class="box-body">
	{!! Form::open(array('url'=>'quotation/delete/', 'class'=>'form-horizontal' ,'id' =>'MmbTable' )) !!}
    ...
    <table class="table table-striped table-bordered " id="{{ $pageModule }}Table">
        ...
        <tbody>
            @foreach ($rowData as $row)
                <tr>
					<td width="30"> {{ ++$i }} </td>
					<td width="50"><input type="checkbox" class="ids minimal-red" name="ids[]" value="{{ $row->quotationID }}" />  </td>
					<td width="100">

						 	@if($access['is_detail'] ==1)
							<a href="{{ url('quotation/show/'.$row->quotationID.'?return='.$return)}}" class="tips" title="{{ Lang::get('core.btn_view') }}"><i class="fa  fa-search fa-2x"></i> </a>
							@endif
							@if($access['is_edit'] ==1)
							<a  href="{{ url('quotation/update/'.$row->quotationID.'?return='.$return) }}" class="tips" title="{{ Lang::get('core.btn_edit') }}"><i class="fa fa-edit fa-2x"></i> </a>
							@endif
							<a  href="{{ url('quotation/show/'.$row->quotationID.'?pdf=true') }}" target="_blank" class="tips text-red" title="PDF"><i class="fa fa-file-pdf-o fa-2x"></i> </a>

					</td>
                    <td><a href="{{ url('quotation/show/'.$row->quotationID.'?return='.$return) }}">{{ $row->quotationID }}</a>
                    </td>
                    ...
                    <td>{!! \App\Library\InvoiceStatus::paymentstatus($row->untildate) !!}
                    </td> 
                    <td><a href="{{ url('invoice/create/'.$row->quotationID) }}" class="btn btn-block btn-xs btn-danger" > Change to Invoice </a></td>
...
$(document).ready(function(){
	...
	$('.do-quick-search').click(function(){
		$('#MmbTable').attr('action','{{ url("quotation/multisearch")}}');
		$('#MmbTable').submit();
	});
	...
	$('.copy').click(function() {
		...
		{
				$('#MmbTable').attr('action','{{ url("quotation/copy")}}');
