...
<?php
    use Carbon\Carbon;
    $payment = DB::table('quotation_payments')->where('quotationID', $row->quotationID )->sum('amount');
    $width= 100*$payment/$row->InvTotal ;
    $maxamount=$row->InvTotal-$payment;
?>
	<div class="box-header with-border">
				<div class="box-header-tools pull-left" >
			   		<a href="{{ url('quotation?return='.$return) }}" class="tips" title="{{ Lang::get('core.btn_back') }}"><i class="fa fa-arrow-left fa-2x"></i></a>
					@if($access['is_add'] ==1)
			   		<a href="{{ url('quotation/update/'.$id.'?return='.$return) }}" class="tips text-green" title="{{ Lang::get('core.btn_edit') }}"><i class="fa  fa-edit fa-2x"></i></a>
					@endif
				</div>
				<div class="box-header-tools pull-right " >
					<a href="{{ ($prevnext['prev'] != '' ? url('quotation/show/'.$prevnext['prev'].'?return='.$return ) : '#') }}" class="tips" title="Previous"><i class="fa fa-arrow-left fa-2x"></i>  </a>
					<a href="{{ ($prevnext['next'] != '' ? url('quotation/show/'.$prevnext['next'].'?return='.$return ) : '#') }}" class="tips" title="Next"> <i class="fa fa-arrow-right fa-2x"></i>  </a>
				</div>
			</div>
...
<php></php>
    <section class="invoice">
      ...
      <div class="row invoice-info">
        ...
        <div class="col-sm-4 invoice-col">
          <h2>{{Lang::get('core.quotation')}}: {{ $row->quotationID}}</h2>
          ...
          <b>{{Lang::get('core.untildate')}}: </b>{!! \App\Library\InvoiceStatus::Paymentstatus($row->untildate) !!}<br>
...
<tr><td></td></tr>
      ...
      <div class="row no-print">
        ...
        @endif
          ...
          <a href="{{ URL::to('quotation/show/'.$id.'?pdf=true') }}" target="_blank"> <button type="button" class="btn btn-primary pull-right" style="margin-right: 5px;">
