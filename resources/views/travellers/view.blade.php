...
<div class="col-md-9">
 ...
 {{Lang::get('core.bookings')}}</a></li>
             <li><a href="#tab_9" data-toggle="tab"><i class="fa fa-square text-green" aria-hidden="true"></i>
            {{Lang::get('core.quotation')}}</a></li>
            ...
            <div class="tab-pane" id="tab_9">
                <table id="invoices" class="table  table-striped">
                    ...
                    <tbody>
                        ...
                        <?php $payment=\DB::table('quotation_payments')->where('quotationID', $quo['quotationID'] )->sum('amount'); ?>
                        <tr>
                            <td>
                                <a href="{{ url('quotation/show/'.$quo['quotationID'])}}" target="_blank">{{ $quo['quotationID'] }}</a>
                            </td>
                            <td><a href="{{ url('createbooking/show/'.$quo['bookingID'])}}" target="_blank">{{ \App\Library\SiteHelpers::formatLookUp($quo['bookingID'],'bookingID','1:bookings:bookingsID:bookingno') }}</a></td>
                            <td>{{ \App\Library\SiteHelpers::TarihFormat($quo['DateIssued'])}}</td>
                            <td><a href="{{ url('invoice/create/'.$quo['quotationID']) }}" class="btn btn-block btn-xs btn-danger" > Change to Invoice </a></td>
                            <td>{!! \App\Library\InvoiceStatus::paymentstatus($quo['untildate'],$quo['status']) !!}</td>
                            <td>{{ \App\Library\SiteHelpers::formatLookUp($quo['currency'],'currencyID','1:def_currency:currencyID:currency_sym') }} {{ $quo['InvTotal'] }} </td>
                        </tr>
                        @endforeach
                    </tbody>
                </table>
