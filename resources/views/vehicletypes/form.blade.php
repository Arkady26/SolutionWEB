...
<div class="box box-primary">
	...
	<div class="box-body">
@endif
			{!! Form::open(array('url'=>'transfer/save/'.\App\Library\SiteHelpers::encryptID($row['vehicleID']), 'class'=>'form-horizontal','files' => true , 'parsley-validate'=>'','novalidate'=>' ','id'=> 'vehicletypesFormAjax')) !!}
			<div class="col-md-12">
...
$(document).ready(function() {
		...
		$("#supplier_id").jCombo("{!! url('transfer/comboselect?filter=def_supplier:supplierID:name&limit=WHERE:status:=:1') !!}",
		{  selected_value : '{{ $row["supplier_id"] }}' });

		$("#currencyID").jCombo("{!! url('transfer/comboselect?filter=def_currency:currencyID:currency_sym|symbol&limit=WHERE:status:=:1') !!}",
	...
	$('input[type="checkbox"],input[type="radio"]').iCheck({
		...
		$('.removeMultiFiles').on('click',function(){
			var removeUrl = '{{ url("transfer/removefiles?file=")}}'+$(this).attr('url');
			$(this).parent().remove();
